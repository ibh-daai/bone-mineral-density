...
def return_findings(
    ...
    fracture_risk = "Cannot be calculated"
    left_femur = report.loc[report.body_part == "Left Femur"]
    if not left_femur.empty:
        neck = left_femur.loc[left_femur.region == "Neck"]
        if not neck.empty:
            ...
            if not female_neck.empty:
                ...
                fracture_risk = get_fracture_risk(
                    t_score,
                    sex,
                    age,
                    lumbar_scores,
    ...
    elif not lumbar_spine.empty:
        if l1_l4 is not None and not l1_l4.empty:
            if age >= 50:
                t_score = l1_l4["t_score"].values[0]
                if t_score <= -2.5:
                    fracture_risk = "Moderate, 10-20%"
                    findings.append(
                        f"10 YEAR ABSOLUTE FRACTURE RISK: {fracture_risk}\n"
                    )
...
def return_summary(
    ...
    lsc = None
    fracture_risk_modifer_statement = None

    if len(change_values) > 0 and not cannot_be_compared:
        change_statement = get_change_statement(change_values)
        lsc = get_lsc(institution_name)

    if fragility_fracture_history == True or glucocorticoid_history == True:
        fracture_risk_modifer_statement = get_fracture_risk_modifer_statement(
    ...
    if risk == "LOW":
        ...
        if fracture_risk_modifer_statement:
            findings.append(fracture_risk_modifer_statement)
        if cannot_be_compared:
            findings.append(
                f"Direct comparison to the prior examination cannot be performed as it was performed on a different machine and any comparison would not be statistically valid.\n\n\
This examination can serve as a baseline for future follow-up examinations, however."
            )
        findings.append(f"Follow-up suggested in 3 years.")
        if lsc:
            findings.append(lsc)
    elif risk == "MODERATE":
        findings.append(f"This patient has {diagnostic} with {risk} FRACTURE RISK.")
        ...
        if fracture_risk_modifer_statement:
            findings.append(fracture_risk_modifer_statement)
        findings.append(
            f"In patients with moderate fracture risk, it would be appropriate to consider a lateral x-ray of the thoracic and lumbar spine from T4-L4 to assess for possible compression fractures."
        )
        findings.append(
            f"The presence of a compression fracture of more than 25% would place the patient into the HIGH RISK category for future fractures and could change management strategies."
        )
        if cannot_be_compared:
            findings.append(
                f"Direct comparison to the prior examination cannot be performed as it was performed on a different machine and any comparison would not be statistically valid.\n\n\
This examination can serve as a baseline for future follow-up examinations, however."
        ...
        findings.append(
            f"Follow-up suggested in 1-3 years, and bone active medication could be considered."
        )
        if lsc:
            findings.append(lsc)
    elif risk == "HIGH":
...
This examination can serve as a baseline for future follow-up examinations, however."
        ...
        findings.append(
            f"Follow-up suggested in 1 years, and bone active medication could be considered."
        )
        if lsc:
            findings.append(lsc)
    elif risk == "WITHIN":
        findings.append(f"This has patient has bone mass {diagnostic}")
        if change_statement:
            findings.append(change_statement)
        findings.append(
...
This examination can serve as a baseline for future follow-up examinations, however."
        ...
        findings.append(f"Follow-up suggested in 1 year.")
        if lsc:
            findings.append(lsc)
    else:
        findings.append(f"This patient has {diagnostic}")
        if change_statement:
            findings.append(change_statement)
        if cannot_be_compared:
            findings.append(
...
def process_sample(
    ...
    study_description = study["description"].values[0]
    study_date = study["date_time"].values[0]
    institution_name = study["institution_name"].values[0]
    ...
    if reference_date_str is not None and baseline_date_str is not None:
        prev_date = pd.to_datetime(reference_date_str).strftime("%B %d, %Y")
        baseline_date = pd.to_datetime(baseline_date_str).strftime("%Y")

    if len(prev_exam_dates) > 2:
        technique = (
            f"A repeat bone density study was obtained on this {age} year old {sex}"
        )
        ...
        technique = (
            f"A baseline bone density study was obtained on this {age} year old {sex}"
        )
        reference_examination = "None."

    if cannot_be_compared == True:
